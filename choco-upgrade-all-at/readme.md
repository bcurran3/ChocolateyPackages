...
### CHANGELOG:
* 0.0.6   - Added option to automatically delete newly created desktop and start menu icons. Added option to run pre and post processing scripts. Edit C:\tools\BCURRAN3\choco-upgrade-all.config to enable. You can now run choco-upgrade-all from the command line to take advantage of these new options. Added choco-install script to work with the same options. Added -EditConfig option to edit the config file. Changed default ABORTTIME to be TIME +2 hours. Added configurable arguments. Added ability to set default user profile.
