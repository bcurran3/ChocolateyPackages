...
if ($bits -eq 64)
   {
	...
	Remove-Item "$toolsDir\x86_64-w64-mingw32\bin\tageditor.exe" #So shim doesn't get created for both
   } else {
    $exeTarget = "$toolsDir\x86_64-w64-mingw32\bin\tageditor.exe"
	Remove-Item "$toolsDir\i686-w64-mingw32\bin\tageditor.exe" #So shim doesn't get created for both
   }
