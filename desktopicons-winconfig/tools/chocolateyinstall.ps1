...
if ($pp["RenameComputer"] -eq "yes" -or $pp["RenameComputer"] -eq "YES" -or $pp["RenameComputer"] -eq "Yes")
     {
      Set-ItemProperty -LiteralPath HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\CLSID\"{20D04FE0-3AEA-1069-A2D8-08002B30309D}" -Name "(Default)" -Value $env:computername
	 } else {
	   if ($pp["RenameComputer"] -eq 'no' -or $pp["RenameComputer"] -eq 'NO' -or $pp["RenameComputer"] -eq 'No')
	     {
	      Set-ItemProperty -LiteralPath HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\CLSID\"{20D04FE0-3AEA-1069-A2D8-08002B30309D}" -Name "(Default)" -Value "This PC"
		 }
	 }

if ($pp["RenameComputer2"] -eq 'yes' -or $pp["RenameComputer2"] -eq 'YES' -or $pp["RenameComputer2"] -eq 'Yes')
     {
	  Set-ItemProperty -LiteralPath HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\CLSID\"{20D04FE0-3AEA-1069-A2D8-08002B30309D}" -Name "(Default)" -Value "$env:computername ($env:username)"
	 } else {
...
if ($pp["OneDrive"] -eq 'yes' -or $pp["OneDrive"] -eq 'YES' -or $pp["OneDrive"] -eq 'Yes')
     {
	  Set-ItemProperty -Path HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\HideDesktopIcons\NewStartPanel -Name "{018D5C66-4533-4307-9B53-224DE2ED1FE6}" -Value 0
	   ...
	   if ($pp["OneDrive"] -eq 'no' -or $pp["OneDrive"] -eq 'NO' -or $pp["OneDrive"] -eq 'No')
	     {
	      Set-ItemProperty -Path HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\HideDesktopIcons\NewStartPanel -Name "{018D5C66-4533-4307-9B53-224DE2ED1FE6}" -Value 1
	     }
	  }	
