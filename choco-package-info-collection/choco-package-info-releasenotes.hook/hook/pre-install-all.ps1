...
# choco-package-info-releasenotes.hook v1.0.0 (2023-07-04) Copyleft 2023 by Bill Curran AKA BCURRAN3
...
if (Test-Path "$env:chocolateyPackageFolder\$env:chocolateyPackageName.nuspec"){
	...
	if ($NuspecReleaseNotes){
		Write-Host "  ** $($NuspecTitle.toupper()) RELEASE NOTES:" -Foreground Cyan
		Write-Host "$(FormatStringWithIndentation "$NuspecReleaseNotes")" -Foreground Cyan
	}
} else {
