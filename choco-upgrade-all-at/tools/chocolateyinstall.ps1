...
$GotTask = (&schtasks /query /tn choco-upgrade-all-at) 2> $null
if ($GotTask -ne $null){
     Write-Host "  ** Existing choco-upgrade-all-at scheduled task found. Keeping existing scheduled task. If you want to change the task runtime or abort time, uninstall and reinstall the package." -foreground magenta 
     exit
   }
...
if (($pp["DAILY"] -eq $null -or $pp["DAILY"] -eq '') -and ($pp["WEEKLY"] -eq $null -or $pp["WEEKLY"] -eq '')){
      ...
      SchTasks /CREATE /SC DAILY /RU $RunAsUser /RL HIGHEST /TN choco-upgrade-all-at /TR "choco upgrade all -y" /ST $RunTime /F
...
if ($pp["WEEKLY"] -eq $null -or $pp["WEEKLY"] -eq ''){
       ...
       if ($pp["DAY"] -eq $null -or $pp["DAY"] -eq ''){
            ...
            SchTasks /CREATE /SC WEEKLY /D SUN /RU $RunAsUser /RL HIGHEST /TN choco-upgrade-all-at /TR "choco upgrade all -y" /ST $RunTime /F
		    SchTasks /query /tn "choco-upgrade-all-at"			
		    ...
		    SchTasks /CREATE /SC WEEKLY /D $pp["DAY"] /RU $RunAsUser /RL HIGHEST /TN choco-upgrade-all-at /TR "choco upgrade all -y" /ST $RunTime /F
		    SchTasks /query /tn "choco-upgrade-all-at"			
