...
if ($bits -eq 64){New-Item "$toolsDir\Log4jScanner\x86\Log4jScanner.exe.ignore" -type file | Out-Null}
if ($bits -eq 32){New-Item "$toolsDir\Log4jScanner\x64\Log4jScanner.exe.ignore" -type file | Out-Null}
